...
class Trainer(object):
    ...
    def mask_out(self, x, lengths):
        ...
        if params.sample_alpha == 0:
            if params.word_pred_uniform:
                # randomly pick src/tgt side that will be used for masking
                side = np.random.randint(0,2,size=[bs,])

                # get src/tgt lengths
                one_pos = (torch.transpose(x,1,0)==1).nonzero()
                assert (one_pos.shape[0] == (x.shape[1] * 4))
                one_pos = one_pos = one_pos.reshape(x.shape[1], 4, 2)
                src_len = (one_pos[:,1][:,1] - one_pos[:,0][:,1]) + 1
                tgt_len = (one_pos[:,3][:,1] - one_pos[:,2][:,1]) + 1
                assert ((lengths - src_len) == tgt_len).all()

                # generate random mask
                pred_mask = np.zeros((slen, bs), dtype=bool)
                while True not in pred_mask:
                    pred_mask = np.random.rand(slen, bs) <= np.random.uniform(0.0, 1.0)
                    # do not predict padding
                    pred_mask[x == params.pad_index] = 0
                    pred_mask[0] = 0  # TODO: remove

                    # reset masks for the other side
                    for batch_idx in range(bs):
                        if side[batch_idx] == 0:
                            pred_mask[src_len[batch_idx]:,batch_idx] = 0
                        elif side[batch_idx] == 1:
                            pred_mask[:src_len[batch_idx],batch_idx] = 0
                        else:
    ...
    def save_periodic(self):
        ...
        if self.params.save_periodic > 0 and self.epoch % self.params.save_periodic == 0:
            if self.params.save_latest_ckpts > 0:
                self.save_model('periodic-%i' % (self.epoch % self.params.save_latest_ckpts))
            else:
