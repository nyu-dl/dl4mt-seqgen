...
from ..utils import create_masked_batch, mask_batch_seq, generate_step
...
class Evaluator(object):
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in set(params.mt_steps + [(l2, l3) for _, l2, l3 in params.bt_steps]):
                    ...
                    refinement_steps = params.refinement_steps
                    scores['%s_%s-%s_mt_bleu' % (data_set, lang1, lang2)] = 0.0
                    if params.encoder_only:
                        lang_pair = params.mlm_steps[0][0] + '-' + params.mlm_steps[0][1]
                        self.evaluate_mt(scores, data_set, lang1, lang2,\
                                        lang_pair, eval_bleu, refinement_steps)
                    else:
    ...
    def evaluate_mt(self, scores, data_set, src_lang, tgt_lang, lang_pair, \
                    eval_bleu, refinement_steps):
        """
        Evaluate BLEU score for TLM (encoder only) model
        Generation is done in left->right order only 1 token at a time
        Using ground-truth length for now!
        """
        params = self.params
        lang1, lang2 = lang_pair.split('-')
        assert data_set in ['valid', 'test']
        assert src_lang in params.langs
        assert tgt_lang in params.langs
        assert lang1 in params.langs
        ...
        src_lang_id = params.lang2id[src_lang]
        tgt_lang_id = params.lang2id[tgt_lang]
        lang1_id = params.lang2id[lang1]
        lang2_id = params.lang2id[lang2]
        if lang1_id == src_lang_id and lang2_id == tgt_lang_id:
            gen_type = "src2tgt"
        elif lang1_id == tgt_lang_id and lang2_id == src_lang_id:
            gen_type = "tgt2src"
        else:
            sys.exit("language pairing is wrong in eval_bleu for TLM model")
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            logger.info('working on batch {}'.format(n_batch))
            n_batch += 1
            # generate batch
            (sent1, len1), (sent2, len2) = batch
            if gen_type == "src2tgt":
                sent1_input = sent1
                sent2_input = create_masked_batch(len2, self.params, self.dico)
                n_iter = torch.max(len2).item()
                dec_len = len2
            elif gen_type == "tgt2src":
                sent1_input = create_masked_batch(len1, self.params, self.dico)
                sent2_input = sent2
                n_iter = torch.max(len1).item()
                dec_len = len1

            x, lengths, positions, langs = concat_batches(sent1_input, len1, lang1_id, \
                                                        sent2_input, len2, lang2_id, \
                                                        params.pad_index, params.eos_index, \
                                                        reset_positions=True)
            # cuda
            x, lengths, positions, langs, len1, len2 = \
                to_cuda(x, lengths, positions, langs, len1, len2)

            # start from index 1
            batch_size = x.size(1)
            cur_idxs = torch.ones(batch_size).long()
            finished_gen = [False] * batch_size
            num_refinements = [0] * batch_size

            # start generation
            while False in finished_gen:
                pred_mask, x, targs = mask_batch_seq(x, len1, len2, self.params,
                                                         right2left=False, start_idxs=cur_idxs,
                                                         finished_gen=finished_gen,
                                                         n_masks_per_step=1, gen_type=gen_type)
                ...
                # abstracted out generation step
                preds = generate_step(scores_pred, topk=1, temperature=1)

                # substitute the predictions back in
                for (seq_idx, batch_idx), pred in zip(pred_mask.nonzero(), preds):
                    if not finished_gen[batch_idx]:
                        x[seq_idx, batch_idx] = pred

                # track where we put the next indices
                for batch_idx in range(batch_size):
                    mod_len = dec_len[batch_idx]
                    # don't generate any token for already finished sentence
                    if not finished_gen[batch_idx]:
                        cur_idxs[batch_idx] = (cur_idxs[batch_idx] + 1) % mod_len

                    # check whether generation is done for this sentence in batch
                    if (finished_gen[batch_idx] == False) and (cur_idxs[batch_idx] == (mod_len - 1)):
                        num_refinements[batch_idx] += 1
                        if num_refinements[batch_idx] == refinement_steps:
                            finished_gen[batch_idx] = True
                            cur_idxs[batch_idx] = 0
                        else:
                            cur_idxs[batch_idx] = 1

            # done with generation (for the batch)
            # convert into text format\
            if eval_bleu:
                for batch_idx in range(batch_size):
                    src_len = len1[batch_idx].item()
                    tgt_len = len2[batch_idx].item()
                    if gen_type == "src2tgt":
                        generated = x[src_len:src_len + tgt_len, batch_idx]
                    else:
                        generated = x[:src_len, batch_idx]
                    # extra <eos>
                    eos_pos = (generated == self.params.eos_index).nonzero()
                    ...
                    hypothesis.extend(convert_to_text(generated.unsqueeze(1), \
                                        ...
                                        self.dico, self.params))

        # compute BLEU
        if eval_bleu:
            # hypothesis / reference paths
            hyp_name = 'hyp{0}.{1}-{2}.{3}.txt'.format(scores['epoch'], \
                                                src_lang, tgt_lang, data_set)
            hyp_path = os.path.join(params.hyp_path, hyp_name)
            ref_path = params.ref_paths[(src_lang, tgt_lang, data_set)]
            ...
            logger.info("BLEU %s-%s; %s %s : %f" % (src_lang, tgt_lang, hyp_path,\
                                                    ref_path, bleu))
            scores['%s_%s-%s_mt_bleu' % (data_set, src_lang, tgt_lang)] = bleu
